(function (window) {
  ...
  function openDeepLink(url, tooltipToggle) {
    const is_ios = /ios|iphone os|iphone|ipod|ipad/i.test(UA)
    const is_firefox = /firefox/i.test(UA)
    const use_iframe = (is_ios || is_firefox)
    const use_once = (!is_ios && is_firefox)
    let timeOpen
    let ttNeedHide = false
    let pageShown = true
  ...
  function packScopeField(field) {
    ...
    if (Array.isArray(field._)) {
      for (let j = 0; j < field._.length; j++)
  ...
  function passportAuth(options, tooltipToggle) {
    ...
    let url = `${is_android ? 'tg:' : 'tg://'}resolve?domain=telegrampassport`
  ...
  function createAuthButton(contEl, authDataOrFunc, options) {
    ...
    options = options || {}
    let tooltipWrapClass
    const btnText = options.text || 'Log In With Telegram'
    const btnRadius = parseInt(options.radius, 10) || 23
    const tooltipText = options.tooltip_text || 'Please <u>install Telegram</u> to use this option.'
    const tooltipForce = options.tooltip_force || false
    const tooltipPos = options.tooltip_position
    if (tooltipPos === 'top')
      tooltipWrapClass = 'tooltip-top'
    else if (tooltipPos === 'left')
      tooltipWrapClass = 'tooltip-left'
    else if (tooltipPos === 'right')
      tooltipWrapClass = 'tooltip-right'
    else
      tooltipWrapClass = 'tooltip-bottom'
    ...
    const btnStyle = ''
      ...
      + 'transform-origin: 0 100%;'
      + '}'
    const tooltipClass = tooltipForce ? 'telegram-passport-tooltip telegram-passport-tooltip-shown' : 'telegram-passport-tooltip'
    const btnHtml = ''
    ...
    const buttonEl = btnContEl.getElementsByTagName('button')[0]
    const tooltipEl = btnContEl.getElementsByTagName('a')[0]
    addEvent(buttonEl, 'click', (e) => {
      preventDefault(e)
      let options = isFunction(authDataOrFunc) ? authDataOrFunc() : authDataOrFunc
      options = options || {}
      const tooltipToggle = function (show) {
        if (show)
